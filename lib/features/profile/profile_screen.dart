...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  Future<void> _loadUserStats() async {
    ...
    print('Loading stats for userId: $userId');
    
    try {
      final reviews = await DatabaseHelper.instance.getAllUserReviews(userId, limit: 1000);
      
      final watchlist = await DatabaseHelper.instance.getWatchlist(userId);
      
      print('User Stats - userId: $userId, Reviews: ${reviews.length}, Watchlist: ${watchlist.length}');

      final user = authProvider.currentUser;
      String joinedYear = '2025';
      if (user?.createdAt != null) {
        try {
          final createdDate = DateTime.parse(user!.createdAt!);
          joinedYear = DateFormat('yyyy').format(createdDate);
        } catch (e) {
          joinedYear = '2025';
        }
      }
                ...
                children: [
                  ...
                  _buildSettingsGroup([
                    ...
                    _buildMenuItem(
                      ...
                      text: 'My Reviews',
                      onTap: () async {
                        await context.push('/profile/my-reviews');
                        _loadUserStats();
                      },
  ...
  Widget _buildStatsRow(
      {required int reviewCount, required int watchlistCount, required String joinedYear}) {
    return Container(
