...
class _ComingSoonScreenState extends State<ComingSoonScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      // The initial list is loaded by fetchAllData in the provider's constructor.
      // We add a listener to fetch more when the user scrolls to the bottom.
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >=
      ...
      // Fetch more movies when the user is 200 pixels from the end
        ...
        (context, index) {
          if (index == movies.length) {
            return provider.isFetchingMoreUpcoming
                ...
                : const SizedBox(height: 40); // Padding at the end
          }
...
class _EnhancedMovieCardState extends State<_EnhancedMovieCard> {
  ...
  String getDaysUntilRelease() {
    ...
    // Normalize dates to midnight to get a clean day count
  ...
  // Hàm mới để lấy màu sắc dựa trên ngày phát hành
  Color _getReleaseDateColor(DateTime? releaseDate) {
    ...
    if (difference < 0) {
      return Colors.grey.shade600; // Đã phát hành
    } else if (difference <= 7) {
      return Colors.redAccent.shade200; // Sắp ra mắt (trong 1 tuần)
    } else if (difference <= 30) {
      return Colors.orangeAccent.shade400; // Sắp ra mắt (trong 1 tháng)
    } else if (difference <= 90) {
      return Colors.blueAccent.shade200; // Sắp ra mắt (trong 3 tháng)
    }
    return Colors.purpleAccent.shade100; // Ra mắt sau 3 tháng
  }
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Stack(
        clipBehavior: Clip.none, // Cho phép badge vẽ ra ngoài
        children: [
          AnimatedContainer(
                            ...
                            children: [
                              ...
                              Text(
                                ...
                                style: TextStyle(
                                    ...
                                    fontSize: 12,
                                    height: 1.4),
                                maxLines: 4, // Tăng số dòng tóm tắt
                                overflow: TextOverflow.ellipsis,
                              ),
                              const Spacer(),
                            ],
          ...
          // Badge được đặt ở đây, bên ngoài vùng bị cắt
  ...
  Widget _buildParallaxPoster(int cacheWidth, int cacheHeight) {
    // Tối ưu hóa: Sử dụng Flow và Transform thay vì AnimatedBuilder và Alignment.
    // Kỹ thuật này hiệu quả hơn vì không cần tính toán lại vị trí toàn cục của widget mỗi lần cuộn.
    return SizedBox(
      width: 120,
      height: 180,
      child: ClipRRect(
...
// Lớp delegate mới để tính toán hiệu ứng parallax một cách hiệu quả.
class _ParallaxFlowDelegate extends FlowDelegate {
  ...
  BoxConstraints getConstraintsForChild(int i, BoxConstraints constraints) {
    // Tăng chiều cao của ảnh nền để có không gian cho hiệu ứng parallax.
    return BoxConstraints.tightFor(
      width: constraints.maxWidth,
      height: constraints.maxHeight * 1.4, // Tăng 40% chiều cao
    );
  }

  @override
  void paintChildren(FlowPaintingContext context) {
    // Lấy RenderBox của item trong danh sách.
    final scrollableBox = scrollable.context.findRenderObject() as RenderBox;
    final listItemBox = listItemContext.findRenderObject() as RenderBox;

    // Tính toán vị trí của item so với viewport.
    ...
    // Dịch chuyển ảnh nền theo chiều dọc để tạo hiệu ứng parallax.
